...
def generate_aes_subkeys(session_key):
    ...
    if sk1[0] & 0b10000000 == 0b10000000:
        sk1 = ((int.from_bytes(sk1, "big") << 1) ^ 0x87).to_bytes(
            block_length + 1, "big"
        )[1:]
    else:
    ...
    if sk1[0] & 0b10000000 == 0b10000000:
        sk2 = ((int.from_bytes(sk1, "big") << 1) ^ 0x87).to_bytes(
            block_length + 1, "big"
        )[1:]
    else:
...
def generate_des_subkeys(session_key):
    ...
    if sk1[0] & 0b10000000 == 0b10000000:
        sk1 = ((int.from_bytes(sk1, "big") << 1) ^ r).to_bytes(
            len(session_key) + 1, "big"
        )[1:]
    else:
        sk1 = (int.from_bytes(sk1, "big") << 1).to_bytes(len(session_key) + 1, "big")[
            1:
        ]

    if sk1[0] & 0b10000000 == 0b10000000:
        sk2 = ((int.from_bytes(sk1, "big") << 1) ^ r).to_bytes(
            len(session_key) + 1, "big"
        )[1:]
    else:
        sk2 = (int.from_bytes(sk1, "big") << 1).to_bytes(len(session_key) + 1, "big")[
            1:
        ]
